...
public class FlowLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // 当前行宽度方向上，已经占用的宽度；比如measureWidth=360，lineWidth=120；
        // 表示在比如measureWidth宽度上，已经占用了120，还剩240；
        int lineWidth = 0;
        int lineHeight = 0;
        int height = 0;
        int width = 0;
        ...
        for (int i=0;i<count;i++){
            ...
            //在强制转换时就会出错，此时我们把左右间距设置为0，但由于在计算布局宽高时没有加上间距值，
            // 就是计算出的宽高要比实际小，所以是onLayout时就会出错
        ...
        // 只有当是wrap_content时，根据内部控件的大小来确定它的大小时，大小是不确定的，属性是AT_MOST,此时，
        // 就需要我们自己计算它的应当的大小，并设置进去
